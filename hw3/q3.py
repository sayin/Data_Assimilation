# -*- coding: utf-8 -*-
"""
Created on Thu Apr 30 20:29:38 2020
...
def plot_lorenz(q,ttrain,X,t,Z,tz,xda=[],tda=0):
    ...
    plt.show()
    fig.tight_layout()
    fig.savefig('bfgs_' +str(q)+ '.pdf', bbox_inches='tight')
...
for q in range(max_iter):
    ...
    I = np.eye(3,3)
    HK = np.linalg.multi_dot([(I - rhok*np.dot(sk,yk.T)), HK, (I - rhok*np.dot(yk,sk.T))]) + rhok*np.dot(sk,sk.T)
